...
public final class ContainerViewController<Output, ScreenType>: UIViewController where ScreenType: Screen & ScreenViewControllerRepresentable {

    /// Emits output events from the bound workflow.
    public let output: Signal<Output, Never>

    internal let rootViewController: ScreenType.ScreenViewControllerType
    ...
    private let context: ScreenType.Context

    private init(workflowHost: Any, rendering: Property<ScreenType>, output: Signal<Output, Never>) {
        self.workflowHost = workflowHost

        let environment = ScreenViewControllerRepresentableContext<ScreenType>.EnvironmentValues()
        self.context = ScreenViewControllerRepresentableContext(environment: environment)
        let vc = rendering.value.makeScreenViewController(context: context)
    ...
    private func render(screen: ScreenType) {
        screen.updateScreenViewController(rootViewController, context: context)
    }
